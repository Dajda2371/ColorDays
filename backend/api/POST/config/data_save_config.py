...
from data_manager import save_main_config_to_json, server_config
...
def save_data_config(payload: ConfigSaveRequest, admin_user: dict = Depends(get_current_admin_user)):
    ...
    # Update the global server config with the new OAuth settings
    server_config.update(payload.dict())
    
    if save_main_config_to_json(server_config):
